...
5. The things I'd like to connect ^[Time permitting]
...
# Create a SliderGrid to control 'a'
slidergrid = SliderGrid(
    fig[2, 1],
    (label = "alpha", range = 0.1:0.1:5.0, startvalue = 1.0),
    tellwidth = false,       # Keeps the slider label width flexible
)

# Access the slider value for 'a'
a_value = slidergrid.sliders[1].value

# Plot the function dynamically using lift
x = LinRange(-2, 2, 500)  # Predefine x values
lines!(ax, x, lift(a_value) do a
    a .* (x .^ 2 .- 1) .^ 2  # Compute y dynamically
end, color = :blue, linewidth = 2)
...
- The metastable stochastic system we sample from follows overdamped Langevin dynamics
\begin{equation}
